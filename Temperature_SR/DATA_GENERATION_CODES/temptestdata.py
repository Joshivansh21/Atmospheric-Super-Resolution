...
for i in range(ERADATA.variables["t2m"].shape[0]):
	Dt=np.array(ERADATA.variables["t2m"][i,:,:])
...
np.save("Highresfiletemptest", ERA_data)

Datain=[[],[],[],[],[]]
for j in range(len(IMDDATA)):
...
np.save("Inputfiletemptest", Input_data)
